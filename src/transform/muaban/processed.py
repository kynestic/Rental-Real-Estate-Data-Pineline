...
def handleArea(area_str):
    ...
    if not isinstance(area_str, str) or not area_str:
        return pd.Series([None, None])
    text = area_str.lower()
    text = re.sub(r'\(.*?\s*[x|×]\s*.*?\)', '', text).strip()
...
def transform_real_estate_df(df_input: pd.DataFrame, parser=None) -> pd.DataFrame:
    """
    Hàm biến đổi dữ liệu Bất động sản theo schema mới (2026).
    """
    
    # 1. Kiểm tra input
    if df_input is None or df_input.empty:
        print("⚠️ DataFrame đầu vào rỗng!")
        return pd.DataFrame()
        
    print(f"⚙️ Đang xử lý {len(df_input)} dòng dữ liệu từ Muaban...")

    # 2. Khởi tạo AddressParser
    # if parser is None:
    #     parser = AddressParser()

    # 3. Tạo DataFrame kết quả mới
    df_out = pd.DataFrame()

    # --- NHÓM 1: LIÊN KẾT DẪN TỚI BẤT ĐỘNG SẢN ---
    df_out['url'] = df_input.get('url')

    # --- NHÓM 2: THÔNG TIN CHÍNH VỀ BẤT ĐỘNG SẢN ---
    df_out['property_type'] = df_input.get('specs.Loại hình bất động sản')
    if 'specs.Giá' in df_input.columns:
        df_out[['num_price', 'unit_price']] = df_input['specs.Giá'].apply(handlePrice)
    # Diện tích sử dụng
    if 'specs.Diện tích sử dụng' in df_input.columns:
        df_out[['num_area', 'unit_area']] = df_input['specs.Diện tích sử dụng'].apply(handleArea)
    # Diện tích đất
    if 'specs.Diện tích đất' in df_input.columns:
        df_out[['num_area', 'unit_area']] = df_input['specs.Diện tích đất'].apply(handleArea)

    # --- NHÓM 3: CẤU TRÚC NHÀ VÀ HƯỚNG ---
    # Số phòng
    cols_mapping_rooms = {
        'specs.Số phòng ngủ': 'num_bedroom',
        'specs.Số phòng vệ sinh': 'num_bathroom',
        'specs.Tổng số tầng': 'floor_count'
    }
    ...
    # Hướng nhà
    # df_out['direction_main'] = df_input.get('specs.Hướng cửa chính').fillna('')
    # df_out['direction_balcony'] = df_input.get('specs.Hướng ban công').fillna('')
    ...
    cols_to_null_value = [
    'iprice', 'eprice', 'wprice'
    ]

    cols_to_null_bool = [
    'camera', 'security', 'pccc', 
    ]

    df_out[cols_to_null_string] = ''
    df_out[cols_to_null_value] = 0
    df_out[cols_to_null_bool] = False

    # --- NHÓM 4: ĐỊA ĐIỂM VÀ TỌA ĐỘ ---
    # Lấy trực tiếp từ place.lat và place.lon thay vì parse từ object 'place' cũ
    df_out['lat'] = pd.to_numeric(df_input.get('place.lat'), errors='coerce')
    df_out['lon'] = pd.to_numeric(df_input.get('place.lon'), errors='coerce')
    # if 'address' in df_input.columns:
    #     print("📍 Đang phân tích địa chỉ...")
    #     address_parsed = df_input['address'].apply(lambda x: pd.Series(parser.parse(x)) if pd.notnull(x) else pd.Series([None]*4))
    #     df_out[['street', 'ward', 'district', 'city']] = address_parsed
    #     df_out['full_address'] = df_input['address']

    # --- NHÓM 5: NỘI DUNG MÔ TẢ VÀ HÌNH ẢNH LIÊN QUAN ---
    if 'description' in df_input.columns:
        df_out['description'] = df_input['description'].apply(handleDescription)
    df_out['img'] = df_input.get('images')
...
# df = pd.read_csv(r'D:\CODING\20251_NguyenManhDuc-DoAnTotNghiep\tmp.csv')

# transform_real_estate_df(df).to_csv('tmp2.csv')